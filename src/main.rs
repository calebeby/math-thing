...
mod tests {
    ...
    fn printing() {
        ...
        insta::assert_debug_snapshot!(exp, @"x * y * π");
        ...
        insta::assert_debug_snapshot!(exp, @"x * y * (y - π) - (x - π)");
    ...
    fn simplify_parens_and_negatives() {
        ...
        insta::assert_debug_snapshot!(exp, @"(-x) * (-y) * (-z)");
        insta::assert_debug_snapshot!(exp.simplify_parens_and_negatives(), @"-x * y * z");
        ...
        insta::assert_debug_snapshot!(exp, @"((-x) * (-y)) * ((-z) * (-x))");
        insta::assert_debug_snapshot!(exp.simplify_parens_and_negatives(), @"(x * y) * (z * x)");

        let exp = math![(-x) * ((-z) * (-x))].expr();
        insta::assert_debug_snapshot!(exp, @"(-x) * ((-z) * (-x))");
        insta::assert_debug_snapshot!(exp.simplify_parens_and_negatives(), @"-x * (z * x)");
    }
}
